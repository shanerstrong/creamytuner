...
import { generateRecipe, getRecipeFixOptions, recommendGuidedRecipe, recommendProgram, validateRecipe, type RecipeFixOption } from '@/src/domain/generator';
...
export default function RecipeBuilderScreen() {
  ...
  useEffect(() => {
    ...
    if (source) { setName(source.name); setPreferences((current) => ({ ...current, style: source.style })); setItems(source.ingredients); setRecommendedAmounts(Object.fromEntries(source.ingredients.map((item) => [item.ingredientId, ingredients.find((ingredient) => ingredient.id === item.ingredientId)?.defaultAmount ?? item.amount]))); setStep(3); }
    initialized.current = true;
  }, [ingredients, ready, source]);
  ...
  const applyRecommendation = () => {
    ...
    const nextItems = [...recommendation.suggestedItems, ...optional].filter((item) => selected.has(item.ingredientId));
    setItems(nextItems);
    setRecommendedAmounts(Object.fromEntries(nextItems.map((item) => [item.ingredientId, item.amount])));
    setAdjustmentIssue(''); setAppliedFix(''); setStep(3);
  };
  const applyTemplate = () => { const template = recipes.find((recipe) => recipe.isTemplate); if (!template) return; setName(template.name); setPreferences((current) => ({ ...current, style: template.style })); setItems(template.ingredients); setRecommendedAmounts(Object.fromEntries(template.ingredients.map((item) => [item.ingredientId, item.amount]))); setAdjustmentIssue(''); setAppliedFix(''); setStep(3); };
  ...
  return (
    <Screen>
      ...
      {step === 4 ? <ReviewStep nutrition={nutrition} validation={validation} program={program} items={items} ingredients={ingredients} settings={settings} fixOptions={fixOptions} appliedFix={appliedFix} onAdjustIssue={(issue) => { setAdjustmentIssue(issue); setAppliedFix(''); setStep(3); }} onApplyFix={applyFix} /> : null}
